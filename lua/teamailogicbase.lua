...
Hooks:PostHook(TeamAILogicBase, "_set_attention_obj", "_set_attention_obj_ub", function(data, att, react)
	...
	-- intimidate
	-- (awake bots in stealth mark but do not intimidate anything, req/bot_stealth.lua)
	if react == AIAttentionObject.REACT_ARREST and not UsefulBots.stealth:holds_fire(data.unit) and (not data._next_intimidate_t or data._next_intimidate_t < data.t) then
