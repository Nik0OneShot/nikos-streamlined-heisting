...
Hooks:PostHook(WeaponTweakData, "init", "sh_init", function(self, tweak_data)
	...
	self.m14_sniper_npc.DAMAGE = 20
	self.m14_sniper_npc.FIRE_MODE = "single"
	self.m14_sniper_npc.single = { fire_rate 60 / 2 } -- this should hopefully make the fire rate not dogshit.
...
function WeaponTweakData:_set_presets()
	...
	for k, v in pairs(self) do
		if k:match("_turret_module") then
			...
			v.HEALTH_INIT = 50 * diff_i -- 500 * diff_i > 50 * diff_i
			v.SHIELD_HEALTH_INIT = 100 * diff_i -- 100 * diff_i > 100 * diff_i
			v.CLIP_SIZE = 50 + 50 * diff_i / 2 -- 300 + 25 * diff_i > 50 + 50 * diff_i / 2
			v.BAG_DMG_MUL = 20
			v.SHIELD_DMG_MUL = 1
			v.FIRE_DMG_MUL = 1
