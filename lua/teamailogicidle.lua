...
function TeamAILogicIdle.is_valid_intimidation_target(other_data, data, distance)
	if data.cool or UsefulBots.settings.dominate_enemies > 2 or UsefulBots.stealth:holds_fire(data.unit) and not data._sh_force_dominate then
		return false
	end
...
function TeamAILogicIdle.on_long_dis_interacted(data, other_unit, secondary, ...)
	...
	if not Keepers and secondary then
		UsefulBots.hold:on_wait_tap(data.unit, UsefulBots:player_settings(other_unit).wait_tap_mode)
...
function TeamAILogicIdle._check_objective_pos(data)
	...
	-- patrolling bots leave their spot to take cover in a fight, the idle update sends them back afterwards
	if data.name == "assault" and UsefulBots.hold:is_patrolling(data) then
		return
	end
...
function TeamAILogicIdle.update(data, ...)
	...
	if not TeamAILogicBase._check_deliver_bag(data) then
		...
		if UsefulBots.hold:update_idle(data) then
			return
		end
